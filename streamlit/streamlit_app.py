...
# Real questions + Bubbles twice + Personalized recovery message + Full reset
...
**Now tell me —**
    ...
    if user_answer.strip():
        ...
        if "PHQ-9" in tool:
            recovery_msg = f"Doing **{user_answer}** is a beautiful step. Small actions like this are proven to lift mood and reduce depression over time. You're building hope, one moment at a time."
        elif "GAD-7" in tool:
            recovery_msg = f"Choosing to do **{user_answer}** helps calm your nervous system and lowers anxiety naturally. You're taking back control — and that’s powerful."
        else:  # WERCAP
            recovery_msg = f"Engaging in **{user_answer}** helps ground you in the present and strengthens your sense of reality. Every positive action reduces psychosis risk and brings clarity."

        st.success("**Uko sawa, utapita hii.**")
        st.markdown(f"**{recovery_msg}**")
        st.markdown("**You are stronger than you know. I'm here whenever you need me.**")
        st.markdown("— MentaBot")

st.markdown("---")
st.caption("Real PHQ-9 • GAD-7 • WERCAP | Bubbles twice | Personalized recovery message | Full Jac code in repo | Eric Jeremiah")
